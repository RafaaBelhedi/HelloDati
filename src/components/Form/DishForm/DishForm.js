...
class DishForm extends Component {
    ...
    if (this.state.alert_message.length == 0) {
      await callApi('post', this.state.data, 'POST').then(res => {
        ...
        if (this.state.tradLang.fr == true) {
          ...
          callApi('posts_translate', translate_data, 'POST').then(res => { console.log(res, "res1") })
        }
        if (this.state.tradLang.ar == true) {
          ...
          callApi('posts_translate', translate_data, 'POST').then(res => { console.log(res, "res2") })
        }
  ...
  render() {
    console.log(this.state.data, "rrrrt")
    return <div>
...
DishForm.contextType = UserContext
