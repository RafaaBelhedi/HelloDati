...
class AddTouristForm extends Component {
	...
	async send() {
		...
		if (!/^[0-9]*$/.test(this.state.tourist.passport_number)) {

			this.setState({ alert_message: [...this.state.alert_message, "Invalid passport_number!"] })
		}
		if (!/^[0-9]*$/.test(this.state.tourist.zip_code)) {
			this.setState({ alert_message: [...this.state.alert_message, "Invalid zip_code!"] })
		}
		...
		if (!/\d{4}-\d{2}-\d{2}/.test(this.state.tourist.born)) {
			...
			this.setState({ alert_message: [...this.state.alert_message, "Invalid Date format ,Please try this format(YYYY-MM-DD) !"] })
		}
		if (this.state.alert_message.length == 0)

			var x = await callApi('tourist', this.state.tourist, 'POST').then(res => { this.setState({ visible: "success" }) }).catch(error => { this.setState({ visible: "echec" }) })
		if (x) {
			firebase
				.database()
				.ref("Tourists")
	...
	render() {
		...
		return <div className='tourist-form-add'>
			...
			<div className="tourist-inputs">
				...
				<input type="date" required placeholder="Born" name="born" onChange={(e) => this.setData({ born: e.target.value })} />
				<input type="number" required placeholder="Phone number" name="phone_number" onChange={(e) => this.setData({ phone_number: e.target.value })} />
				<input type="number" required placeholder="CIN Number" name="cin_number" onChange={(e) => this.setData({ cin_number: e.target.value })} />
